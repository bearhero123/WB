...
async def run_checkin(db: AsyncSession, account: Account) -> dict:
    ...
    if not topics:
        ...
        stats = {"total": 0, "success": 0, "already": 0, "failed": 0, "cookie_valid": True}
    ...
    # 3. 遍历签到
    stats = {"total": len(topics), "success": 0, "already": 0, "failed": 0, "failed_items": []}
    ...
    for topic in topics:
        ...
        if result:
            ...
            logger.info(f"  [{result.status}] {topic.title}: {result.detail}")
